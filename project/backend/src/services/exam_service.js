...
async function FindExamsByCourseId(course_id) {

    try {
        const db = mongoose.connection.db;
        const exams = await db.collection("exams").find({ in_course_id: parseInt(course_id) }).toArray();
        return exams;
    }
    catch (error) {
        console.error("Error finding exams by course ID:", error);
    }
}
