...
import { createEditor, Editor, Transforms, Element as SlateElement, Text, Range } from "slate";
...
const toggleMark = (editor, format) => {
    ...
    if (!isPreview && markers[format]) {
        const [open, close] = markers[format];
        const { selection } = editor;
        if (selection && !Range.isCollapsed(selection)) {
            Transforms.insertText(editor, close, { at: selection.focus });
            Transforms.insertText(editor, open, { at: selection.anchor });
        }
        return;
    }
...
const isBlockActive = (editor, format) =>
    !!Editor.nodes(editor, { match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === format })[0];
...
const Element = ({ attributes, children, element }) => {
    switch (element.type) {
        ...
        default:
            return (
                <p {...attributes} className="my-2 leading-7">
                    {children}
                </p>
            );
    }
...
const ToolbarButton = ({ format, icon: Icon }) => {
    ...
    return (
        <Button
            ...
            onMouseDown={e => {
                e.preventDefault();
                ["bold", "italic", "code"].includes(format) ? toggleMark(editor, format) : toggleBlock(editor, format);
            }}
            aria-label={format}
        >
            <Icon size={16} />
...
const Toolbar = () => (
    ...
    >
        {[
            ...
            ["heading", Heading],
            ["quote", Quote],
            ["bulleted-list", ListIcon],
            ["numbered-list", ListOrdered],
        ].map(([fmt, Icon]) => (
            <ToolbarButton key={fmt} format={fmt} icon={Icon} />
        ))}
    </motion.div>
);
    ...
    nodes
        .map(n => {
            if (Text.isText(n)) {
                let txt = n.text;
                if (n.code) txt = `\`${txt}\``;
                if (n.bold && n.italic) txt = `***${txt}***`;
                else if (n.bold) txt = `**${txt}**`;
                else if (n.italic) txt = `*${txt}*`;
                return txt;
            }
...
export default function GitHubCommentEditor({ onSubmit, onTextChange }) {
    ...
    const handleKeyDown = useCallback(
        e => {
            for (const hot in HOTKEYS) if (isHotkey(hot, e)) { e.preventDefault(); toggleMark(editor, HOTKEYS[hot]); }
            if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) { e.preventDefault(); onSubmit && onSubmit(toMarkdown(value)); }
        },
        [editor, value, onSubmit]
    );
    return (
